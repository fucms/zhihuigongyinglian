    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="单位名称">
          <el-input v-model="listQuery.filter" placeholder="请输入单位名称" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="单位名称" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code1 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="助记码" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code2 }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="注册资本" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="法人代表" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code5 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="注册日期" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code6 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="启用状态" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code7 }}</span>
          </template>
        </el-table-column>
