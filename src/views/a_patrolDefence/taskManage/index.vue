  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="单据号ID">
          <el-input v-model="listQuery.filter1" placeholder="请输入单据号ID" />
        </el-form-item>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="单据号ID" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code1 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="日期" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code2 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="供货方" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="采购方" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="付款方式" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code5 }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="重量" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code7 }}</span>
          </template>
        </el-table-column>
