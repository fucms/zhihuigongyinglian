    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="司机ID">
          <el-input v-model="listQuery.filter1" placeholder="请输入司机ID" />
        </el-form-item>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="司机ID" prop="code" align="center">
        ...
        <el-table-column label="编码" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="登录名" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="密码" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code5 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="手机号" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code6 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="用户类型" prop="type3" align="center">
          <template slot-scope="{ row }">
            <el-tag :type="row.type1 == 1? 'success' : 'info'">{{ row.type1 == 1? '司机' : '操作员' }}</el-tag>
          </template>
        </el-table-column>
