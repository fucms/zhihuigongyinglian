  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="事项ID">
          <el-input v-model="listQuery.filter1" placeholder="请输入事项ID" />
        </el-form-item>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="事项ID" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code1 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="事项名称" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code2 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="事项内容" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="事项完成时间" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="事项等级" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code5 }}</span>
          </template>
        </el-table-column>
