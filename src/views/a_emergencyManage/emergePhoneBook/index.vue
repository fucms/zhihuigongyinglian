    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="ID">
          <el-input v-model="listQuery.filter1" placeholder="请输入ID" />
        </el-form-item>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="机构ID" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code1 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="机构简称" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code2 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="电话" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code3 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="传真" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code4 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="法人" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code5 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="税号" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code6 }}</span>
          </template>
        </el-table-column>
        <el-table-column label="开户行" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code7 }}</span>
          </template>
        </el-table-column>
